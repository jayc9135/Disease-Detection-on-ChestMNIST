...
class FlowerClient(fl.client.NumPyClient):
    def __init__(self, cid, poisoned=False, flip_ratio=0.75):
        ...
        # Model initialization with 9 classes for PathMNIST
        self.model = CentralCNN(in_channels=3, n_classes=9).to(self.device)
        ...
        # Loss function and optimizer
        self.loss_function = nn.CrossEntropyLoss()  # Use CrossEntropy for multi-class
        self.optimizer = optim.Adam(self.model.parameters(), lr=0.0001)
        self.accuracy = Accuracy(task='multiclass', num_classes=9).to(self.device)
    ...
    def fit(self, parameters, config):
        ...
        for batch_idx, (images, labels) in enumerate(self.train_loader):
            ...
            running_loss += loss.item()
            self.accuracy.update(outputs.argmax(dim=1), labels)
