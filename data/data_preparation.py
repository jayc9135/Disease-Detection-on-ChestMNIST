...
def _check_labels(dataset, binary=True):
    """
...
def clean_data(dataset, binary_labels=True):
    # Skip binary label check if the dataset is multi-class
    _check_labels(dataset, binary=binary_labels)
    dataset = _remove_duplicate_values(dataset)
    dataset = _noise_removal(dataset)
    return dataset
